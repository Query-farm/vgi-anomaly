...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-anomaly
...
# ATTACH (`${VGI_ANOMALY_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run anomaly_worker.py`) — the
...
#                        `uv run --python 3.13 <repo>/anomaly_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/anomaly_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_ANOMALY_WORKER (the LOCATION) and, for the
...
case "$TRANSPORT" in
  subprocess)
    export VGI_ANOMALY_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/anom-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/anom-http-server.log"
    ...
    export VGI_ANOMALY_WORKER="http://127.0.0.1:$PORT"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/anom-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/anom-unix-server.log"
    ...
    export VGI_ANOMALY_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_ANOMALY_WORKER) ..."
