# CI: the vgi-anomaly worker integration suite

[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs the unit tests
and this repo's sqllogictest suite (`test/sql/*.test`) against the vgi-anomaly
...
1. **Install the worker** — `uv sync --frozen` into a venv. `anomaly_worker.py`
   is a self-contained PEP 723 stdio worker the extension can spawn via
   `uv run anomaly_worker.py`.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_ANOMALY_WORKER` at `uv run anomaly_worker.py`, warms the
...
VGI_ANOMALY_WORKER="uv run --python 3.13 anomaly_worker.py" \
  ci/run-integration.sh
```

Or use the Makefile target `make test-sql`, which installs `haybarn-unittest`
as a uv tool and points the worker at `uv run --python 3.13 anomaly_worker.py`.
